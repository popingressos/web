...
$retorno_verifica_whatsapp = verificaWhatsApp("".$rSqlEmpresa['id']."","".$rSqlCarrinho['pessoa_telefone']."");

if(trim($retorno_verifica_whatsapp)=="SIM") {
	if(trim($rSqlCarrinho['pessoa_telefone'])=="") { } else {
		if(trim($rSqlSession['session'])=="") { } else {
			#NOVO ENVIO DE WHATSAPP
			$rSqlSend['modelo_de_envio_texto'] = 1;
			$rSqlSend['modelo_de_envio_pessoa'] = 1;
			$rSqlSend['modelo_de_envio_evento'] = 1;
			$rSqlSend['com_qrcode'] = 1;
			$rSqlSend['whatsapp'] = $rSqlSend['whatsapp'];
			$rSqlSend['titulo'] = $rSqlSend['assunto'];
			$rSqlSend['nome'] = "".$nomeWhatsSet."";
			$rSqlSend['email'] = "".$rSqlCarrinho['pessoa_email']."";
			$rSqlSend['documento'] = "".$documentoWhatsSet."";
			$rSqlSend['evento_nome'] = "".$evento_nomeWhatsSet."";
			$rSqlSend['ingresso_nome'] = "".$rSqlEnvio['ingresso_nome']."";
			$rSqlSend['ingresso_data'] = "".$rSqlEnvio['ingresso_data']."";
			if(trim($rSqlCarrinho["label"])=="" && trim($rSqlCarrinho["linha_real"])=="" && trim($rSqlCarrinho["coluna_real"])=="") { 
			$rSqlSend['ingresso_cadeira'] = "";
			} else {
			$rSqlSend['ingresso_cadeira'] = $rSqlCarrinho['label'];
			}
			$rSqlSend['lote_nome'] = "";
